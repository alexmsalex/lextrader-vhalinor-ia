...
class AdvancedEnvironmentValidator:
    ...
    def check_required_packages(self) -> Dict[str, Any]:
        ...
        # Verifica pacotes requeridos (robusto contra diferenças de case)
        for package, min_version in required_packages.items():
            key = package.lower()
            if key not in installed_packages:
                # Se não requer versão mínima, é presumivelmente built-in
                ...
                current_version = installed_packages.get(key)
                if min_version and current_version and current_version < min_version:
                    ...
                    package_results["required"]["ok"].append({
                        "package": package,
                        "version": current_version or "built-in"
                    })
        
        # Verifica pacotes opcionais (robusto contra diferenças de case)
